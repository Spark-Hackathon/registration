...
router.post("/admin/export/week", async (req, res, next) => {
	try {
		await admin_validate(req.body.code);
		connection.query("SELECT *, (SELECT COUNT(*) FROM medical_forms WHERE medical_forms.camper_id=camper.id) AS med_status, (SELECT COUNT(*) FROM consent_release WHERE consent_release.camper_id=camper.id) AS consent_status FROM camper INNER JOIN enrollment ON enrollment.camper_id = camper.id WHERE enrollment.week_id=?", week_meta.get(req.body.week_name).id, (err, week_campers) => {
			if (err) throw err;
			for (i in week_campers) {
				week_campers[i].confirmed = week_campers[i].person_loc ? week_campers[i].med_status == 1 && week_campers[i].consent_status == 1 : week_campers[i].consent_status == 1;
